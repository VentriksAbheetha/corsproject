...
export async function cors() {
    ...
    for (let i = 0; i < qaAPIS.item.length; i++) {
        for (let j = 1; j < qaAPIS.item[i].item.length; j++) {
            // console.log(qaAPIS.item[i].item[0].request.url.path[0])
            if (qaAPIS.item[i].name === ('Calendars')) { //Name of the module here can be changed to check the response of other modules
